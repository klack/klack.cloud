...
if [ -z "$PLEX_CLAIM" ]; then
    ...
    tar -xzf ./config/plex/provision.tar.gz -C $DIR_META_DATA_ROOT
    cp ./config/plex/Preferences.xml.template "$DIR_META_DATA_ROOT/plex/Library/Application Support/Plex Media Server/Preferences.xml"
    sed -i "s/\${EXTERNAL_DOMAIN}/${EXTERNAL_DOMAIN}/g" "$DIR_META_DATA_ROOT/plex/Library/Application Support/Plex Media Server/Preferences.xml"
    sed -i "s/\${CLOUD_USER}/${CLOUD_USER}/g" "$DIR_META_DATA_ROOT/plex/Library/Application Support/Plex Media Server/Preferences.xml"
