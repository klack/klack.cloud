...
# Wait for the SFTPGo service to respond with 401 Unauthorized
echo "Waiting for SFTPGo service to return 401 Unauthorized..."
...
until [[ "$(curl -k -s -o /dev/null -w '%{http_code}' $CHECK_URL -k)" == "200" ]]; do
    ...
    echo "Service is not yet responding with 200. Retrying in $RETRY_INTERVAL seconds..."
    sleep $RETRY_INTERVAL
done

echo "SFTPGo is responding with 200. Proceeding with user creation."
