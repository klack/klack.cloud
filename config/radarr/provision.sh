...
curl "$SERVER/api/v3/downloadclient?" \
    ...
    --data-raw '{
        ...
        "fields": [
            ...
            {"name": "urlBase"},
            {"name": "username", "value": "'"$CLOUD_USER"'" },
            {"name": "password", "value": "'"$CLOUD_PASS"'" },
...
curl "$SERVER/api/v3/indexer?" \
    ...
    -H "X-Api-Key: $RADARR_API_KEY" \
    -H "Authorization: Basic $BASIC_AUTH_BASE64" \
    --data-raw '{"enableRss":true,"enableAutomaticSearch":true,"enableInteractiveSearch":true,"supportsRss":true,"supportsSearch":true,"protocol":"torrent","priority":25,"downloadClientId":0,"name":"YTS","fields":[{"name":"baseUrl","value":"http://localhost:9117/api/v2.0/indexers/yts/results/torznab/"},{"name":"apiPath","value":"/api"},{"name":"apiKey","value":"'$JACKETT_API_KEY'"},{"name":"categories","value":[2000,2040,2045,2060]},{"name":"additionalParameters"},{"name":"multiLanguages","value":[]},{"name":"removeYear","value":false},{"name":"minimumSeeders","value":0},{"name":"seedCriteria.seedRatio"},{"name":"seedCriteria.seedTime"},{"name":"rejectBlocklistedTorrentHashesWhileGrabbing","value":false},{"name":"requiredFlags","value":[]}],"implementationName":"Torznab","implementation":"Torznab","configContract":"TorznabSettings","infoLink":"https://wiki.servarr.com/radarr/supported#torznab","tags":[]}' \
    -s -o /dev/null -w '%{http_code}\n'
